...
@OnlyIn(Dist.CLIENT)
public class FeatherMoteParticle extends TextureSheetParticle {

    private static final int FADE_TICKS = 18;

    private static final double[] WIND = new double[3];
    ...
    protected FeatherMoteParticle(ClientLevel level, double x, double y, double z,
        ...
        quadSize = (float) RandUtils.rangef(0.055f, 0.105f);

        gravity = 0.16f;
        friction = 0.94f;

        hasPhysics = true;
        lifetime = RandUtils.rangei(55, 96);

        settleYaw = (float) (RandUtils.nextDouble() * Math.PI * 2);

        roll = oRoll = (float) (RandUtils.nextDouble() * Math.PI * 2);
        rollSpeed = (float) RandUtils.rangef(-0.13f, 0.13f);

        swayFreq = RandUtils.rangef(0.14f, 0.32f);
        swayAmp = RandUtils.rangef(0.0020f, 0.0042f);
    ...
    public void tick() {
        ...
        if (settleAge < 0) {
            ...
            if (WingWind.push(x, y, z, level.getGameTime(), WIND)) {
                xd += WIND[0];
                yd += WIND[1];
                zd += WIND[2];
            }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType type, ClientLevel level,
            ...
            return new FeatherMoteParticle(level, x, y, z, vx, vy, vz, sprites);
        }
    }
}
